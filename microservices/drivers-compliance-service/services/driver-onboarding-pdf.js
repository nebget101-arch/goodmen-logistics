...
function resolveTemplatePath(relativePath) {
  const localPath = path.join(__dirname, '../assets/templates', relativePath);
  if (fs.existsSync(localPath)) return localPath;
  return path.join(
    __dirname,
    '../../../goodmen-logistics/backend/assets/templates',
    relativePath
  );
}
...
const employmentTemplatePath =
  ...
  resolveTemplatePath('Drivers_Employment_Application_508.pdf');

const mvrTemplatePath =
  process.env.MVR_AUTHORIZATION_TEMPLATE_PATH ||
  resolveTemplatePath('MVR_-_Employee_Authorization_Form.pdf');
