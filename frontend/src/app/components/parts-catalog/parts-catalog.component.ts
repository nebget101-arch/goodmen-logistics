...
export class PartsCatalogComponent implements OnInit, OnDestroy {
  ...
  // ── FN-1399: Print Label flow ───────────────────────────────────────────────
  /** Part shown by the success modal after a successful create. Null = modal closed. */
  printablePart: any = null;
  /** Part fed into the hidden <app-part-label-print> instance for window.print(). */
  partForLabel: any = null;
  /**
   * FN-1398 acceptance: form barcode is auto-generated by the backend by
   * default. Users may opt in to a vendor-supplied barcode via this toggle,
   * which unlocks the input. Reset when the modal opens/closes.
   */
  vendorBarcodeOverride = false;
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  openForm(part?: any): void {
    ...
    // FN-1399: vendor-barcode override is opt-in per modal session.
    this.vendorBarcodeOverride = false;
  }
    ...
    if (this.editingPartId) {
      ...
      this.apiService.createPart(formData).subscribe({
        next: (response: any) => {
          ...
          // FN-1399: Capture the persisted part (with backend-generated barcode)
          // so the success modal can offer a Print Label action. The BE response
          // shape is { data: { …part } } per parts.service.createPart.
          const created = response?.data ?? response;
          this.loadParts();
          this.closeForm();
          this.openPrintConfirmModal(created);
          setTimeout(() => this.successMessage = '', 3000);
          this.loading = false;
        },
        error: (error: any) => {
  ...
  /**
   * Show the post-create success modal with a "Print label" CTA. Skips the
   * modal if the persisted part has no barcode value (defensive — backend
   * always generates one in FN-1400, but we don't want a half-broken modal
   * if the FE is run against a pre-FN-1400 backend).
   */
  private openPrintConfirmModal(part: any): void {
    if (!part || !part.barcode) {
      this.printablePart = null;
      return;
    }
    this.printablePart = part;
  }

  /** Dismiss the success modal without printing. */
  dismissPrintConfirm(): void {
    this.printablePart = null;
  }

  /**
   * Hand the part to the hidden label component, wait one tick for it to
   * render the SVGs, then trigger the browser print dialog. Mirrors the
   * `drivers.component.ts:printDqfReport` pattern — print styles in the
   * label component CSS hide the rest of the app via @media print.
   */
  printLabelForPart(part: any): void {
    if (!part || !part.barcode) return;
    this.partForLabel = part;
    // Wait for the @ViewChild to render the SVGs; JsBarcode runs on
    // ngAfterViewInit / ngOnChanges, so a single macrotask is enough.
    setTimeout(() => {
      window.print();
      // Keep the partForLabel mounted briefly so the print dialog snapshot
      // has the rendered SVGs, then clear it. The success modal is closed
      // in the same handler so the user lands back on the catalog table.
      setTimeout(() => {
        this.partForLabel = null;
      }, 500);
    }, 50);
    this.printablePart = null;
  }
