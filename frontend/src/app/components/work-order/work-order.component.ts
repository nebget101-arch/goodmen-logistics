  ...
  applyAiTriage(): void {
    ...
    if (Array.isArray(result.parts)) {
      for (const p of result.parts) {
        ...
        const match = this.partsCatalog.find(part => {
          ...
          if (queryWords.length > 0 && queryWords.every(w => name.includes(w) || sku.includes(w))) return true;
          return false;
        });
        if (match) {
