...
async function main() {
  ...
  // Sample query 2: "delivered loads from TX to FL last month"
  // Date math is performed by the model; the validator passes concrete
  // ISO dates through unchanged.
  ...
  // Sample query 3: "loads for broker CH Robinson under $2500"
  ...
  // Sample query 4: "load number ABC-123"
  ...
  // Sample query 5: gibberish -> empty model output -> fallback
  ...
  // AC: unknown fields must be dropped by the validator.
  ...
  // Unparseable model text -> fallback with reason unparseable_model_output.
  ...
  // Direct validator unit checks.
  {
    // Bad enum dropped, good state uppercased, bad rate dropped.
