...
router.get('/compliance-summary', async (req, res) => {
  try {
    ...
    const hosViolationsRes = await query(`SELECT COUNT(*) as count FROM hos_records hr JOIN drivers d ON hr.driver_id = d.id ${hosWhereClause} ${hosWhereClause ? 'AND' : 'WHERE'} array_length(violations, 1) > 0`, hosParams);
    const hosWarningsRes = await query(`SELECT COUNT(*) as count FROM hos_records hr JOIN drivers d ON hr.driver_id = d.id ${hosWhereClause} ${hosWhereClause ? 'AND' : 'WHERE'} status = 'warning'`, hosParams);
    const hosCompliantRes = await query(`SELECT COUNT(*) as count FROM hos_records hr JOIN drivers d ON hr.driver_id = d.id ${hosWhereClause} ${hosWhereClause ? 'AND' : 'WHERE'} status = 'compliant'`, hosParams);
