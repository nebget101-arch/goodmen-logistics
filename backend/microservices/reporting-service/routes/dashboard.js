...
const db = require('@goodmen/shared/config/knex');
  ...
  try {
    const stats = await query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM loads WHERE UPPER(status::text) IN ('DISPATCHED', 'dispatched')) as "loadsDispatched",
        (SELECT COUNT(*) FROM loads WHERE UPPER(status::text) IN ('IN_TRANSIT', 'IN-TRANSIT', 'in-transit')) as "loadsInTransit",
        ...
        (SELECT COUNT(*) FROM loads WHERE UPPER(COALESCE(billing_status::text, 'PENDING')) = 'PENDING') as "billingPending",
        (SELECT COUNT(*) FROM loads WHERE UPPER(COALESCE(billing_status::text, 'PENDING')) = 'FUNDED') as "billingFunded",
        (SELECT COUNT(*) FROM loads WHERE UPPER(COALESCE(billing_status::text, 'PENDING')) = 'INVOICED') as "billingInvoiced",
        (SELECT COUNT(*) FROM loads WHERE UPPER(COALESCE(billing_status::text, 'PENDING')) = 'PAID') as "billingPaid",
