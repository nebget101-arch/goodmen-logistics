...
/**
 * FMCSA Bulk Importer (FN-1414 / FN-1422).
 *
 * Streams FMCSA snapshot files (Inspection, Crash, SMS BASIC scores) into
 * the `fmcsa.*` schema via a Bull queue with three job processors:
 *
 *   - import-fmcsa-inspections
 *   - import-fmcsa-crashes
 *   - import-fmcsa-sms
 *
 * Job data shape (all three):
 *   {
 ...
 * processors; callers enqueue jobs explicitly via the returned helpers.
 */
...
/**
 * Construct the FMCSA import queue and register all 3 processors.
 *
...
module.exports = {
  createImportQueue,
  // Re-export drivers and parsers for direct use (e.g. tests, ad-hoc CLI runs)
  runInspectionImport,
  runCrashImport,
  runSmsImport,
  parsers: {
