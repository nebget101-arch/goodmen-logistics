...
async function handlePaymentSucceeded(event) {
  ...
  try {
    ...
    // Mark trial as converted if still active
...
async function handlePaymentFailed(event) {
  ...
  try {
    ...
    // Set a 3-day grace period for payment recovery
    ...
    // Trigger payment failure email (FN-76 will implement sendPaymentFailureEmail)
    // For now, just log it; the actual email send will be handled by FN-76
...
async function routeEvent(event) {
  switch (event.type) {
    ...
    case 'customer.subscription.updated':
      ...
      break;
    default:
      // intentionally ignore unknown event types
      break;
  }
}
...
router.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  // Return immediately so Stripe does not retry while async handlers run.
